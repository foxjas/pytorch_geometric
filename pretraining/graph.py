...
class Graph(defaultdict):
  ...
  def __eq__(self, graph):
    """
    Two graphs are equal if their node and edge counts identical,
    and if every edge that exists in one graph also exists in other.
    """
    other_num_vertices = graph.number_of_nodes()
    other_num_edges = graph.number_of_edges()
    same_counts = other_num_vertices == self.number_of_nodes() and \
                  other_num_edges == self.number_of_edges()
    if not same_counts:
        return False

    same_graph = True
    for src, adjacency in self.adjacency_iter():
...
def write_normal_randomwalks(G, file_, num_paths = 50, path_length = 5,
  ...
  nodes = list(G.nodes())
  pairs = []
  makedirs(path.dirname(file_), exist_ok=True)
...
def build_deepwalk_corpus(G, num_paths, path_length, alpha=0,
  ...
  print("max node in G: {}".format(max(nodes)))
  for cnt in range(num_paths):
    rand.shuffle(nodes)
    for node in nodes:
...
def load_edgelist(num_vertices, file_, undirected=True):
  G = Graph()
