...
Adapted from: https://github.com/snap-stanford/graphwave
"""
import math
...
def build_structure(width_basis, basis_type, list_shapes, start=0,
    ...
    '''
    basis, role_id = basis_type(start, width_basis)
    ...
    seen_shapes = {'basis': [0, n_basis]} # 2nd entry not used
    for p in plugins:
        role_id[p] += 1
    ...
    for shape_id, shape in enumerate(list_shapes):
        ...
        basis.add_edges_from([(start, plugins[shape_id])]) # bridge to shape
        role_id[plugins[shape_id]] += (-2 - 10 * seen_shapes[shape_type][0]) # plugin node (basis)
        #role_id[plugins[shape_id]] += (-100+seen_shapes[shape_type][0]) # plugin node (basis)
        communities += [shape_id] * n_s
        temp_labels = [r + col_start for r in roles_graph_s]
        temp_labels[0] += 100 * seen_shapes[shape_type][0] # bridge node (shape)
        #temp_labels[0] = 100+seen_shapes[shape_type][0] # bridge node (shape)
        role_id += temp_labels
        start += n_s
